...
╚════════════════════════════════════════════╝
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # 限制关节范围
            self.boom_pos = max(-0.5, min(1.2, self.boom_pos))
            self.arm_pos = max(-2.0, min(0.5, self.arm_pos))
            ...
            # 显示当前位置
            sys.stdout.write(f"\r大臂:{self.boom_pos:+.2f} 小臂:{self.arm_pos:+.2f} 铲斗:{self.bucket_pos:+.2f} 回转:{self.swing_pos:+.2f}  ")
            sys.stdout.flush()
